import re
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer, LlamaForCausalLM, LlamaTokenizerFast, LlamaTokenizer
...
class ModelAndTokenizer:
    ...
    def __init__(
        ...
        if tokenizer is None:
            ...
            if any(sub in model_name for sub in ['pmc', 'menta']):
                tokenizer = LlamaTokenizer.from_pretrained(model_name,legacy=False)
            elif 'palmyra' in model_name:
                tokenizer = AutoTokenizer.from_pretrained(model_name, use_fast=False)
            elif 'fin' in model_name:
                tokenizer = LlamaTokenizerFast.from_pretrained("/root/autodl-tmp/project/llama", trust_remote_code=True)
            else:
                tokenizer = AutoTokenizer.from_pretrained(model_name)
        ...
        if load_8_bit:
            ...
            # max_memory = {int(cuda): '18GiB' for cuda in device.split(':')[1].split(",")}
            max_memory = {int(cuda): '30GiB' for cuda in device.split(':')[1].split(",")}

            if  'fin' in model_name:
                base_model = LlamaForCausalLM.from_pretrained(
                    "/root/autodl-tmp/project/llama",
...
def generate_hidden_states(current_test_cases , mt, layer_range=range(18,32), device='cuda:0', max_new_tokens=100, tok=-1):
    with torch.no_grad():
        tokenized_input = mt.tokenizer(current_test_cases, padding=True, return_tensors="pt").input_ids.to(device)
        hidden_st = mt.model(tokenized_input, output_hidden_states=1).hidden_states
